...
double apply_discount(double before_discount, double discount) {
    ...
    double reduce = before_discount * discount / 100;
    return before_discount - reduce;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double mRate = daily_rate(hourly_rate) * 22;
    mRate = apply_discount(mRate, discount);
    return ceil(mRate);
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double dis = apply_discount(hourly_rate, discount);
    double dRate = daily_rate(dis);
    
    return floor(budget/dRate);
}